  ...
  if (members.length === 0) {
    return (
      <div className="glass rounded-2xl p-8 text-center text-muted-foreground">
        No approved members in your community yet.
      </div>
    );
  }
      ...
      {members.map((m) => (
        <div key={m.id} className="glass rounded-xl p-5 flex gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            {/* Show contact ONLY for admin */}
            {isAdmin && (
              <>
                <a
                  href={`mailto:${m.gmail}`}
                  className="mt-2 flex items-center gap-1.5 text-xs text-muted-foreground hover:text-primary truncate"
                >
