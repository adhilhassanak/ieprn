...
export const CommunityLogosPanel = () => {
  ...
  const upload = async (communityKey: CommunityKey, file: File) => {
    ...
    try {
      ...
      const { error: dbErr } = await supabase
        ...
        .upsert({ community: communityKey, logo_url: url }, { onConflict: "community" });
      if (dbErr) throw dbErr;

      toast({ title: `${communityKey} logo updated` });
      await refresh();
    } catch (e: any) {
  ...
  const remove = async (communityKey: CommunityKey) => {
    ...
    try {
      const { error } = await supabase
        .from("community_logos")
        .upsert({ community: communityKey, logo_url: null }, { onConflict: "community" });
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold">Community Logo Management</h2>
